...
def load_config(path: Path) -> dict[str, Any]:
    ...
    # 1) {"wechat": {...}}
    # 2) {"platforms": {"wechat": {...}}}
    # 3) feishu2wechat 双版本配置：{"_comment": "当前使用版本：test", "test": {"wechat": {...}}}
    if isinstance(data.get("wechat"), dict):
        return data
    if isinstance(data.get("platforms", {}).get("wechat"), dict):
        return {"wechat": data["platforms"]["wechat"]}
    version = data.get("_comment", "").replace("当前使用版本：", "").strip()
    if version not in {"test", "production"}:
        version = "test"
    if isinstance(data.get(version, {}).get("wechat"), dict):
        return {
            "wechat": {
                **data[version]["wechat"],
    ...
    for p in soup.find_all("p"):
        ...
        if template == "viral":
            p["style"] = (
                ...
                "border-radius:8px;line-height:2;font-size:17px;color:#2b2f38;text-indent:0;"
...
def _callout_style_key(text: str) -> str | None:
    ...
    if text.startswith(("📌", "📊")):
        return "note"
    if text.startswith("👉") or "一句话总结" in text or "一句话概括" in text or "真正贡献" in text:
        return "key"
    return None
...
def _is_chinese_enum_point(text: str) -> bool:
    return bool(re.match(r"^第[一二三四五六七八九十]+(?:类)?(?:[，、：:。.]|是)", text.strip()))


def _split_br_fragments(node) -> list[str]:
    fragments = []
    current = []
    for child in node.children:
        if getattr(child, "name", None) == "br":
            fragment = "".join(str(item) for item in current).strip()
            if fragment:
...
def _split_chinese_enum_paragraphs(soup: BeautifulSoup) -> None:
    for p in list(soup.find_all("p")):
        if p.find_parent(["li", "blockquote", "td", "th"]):
            continue
        fragments = _split_br_fragments(p) if p.find("br") else [p.decode_contents().strip()]
        if not fragments or not any(_is_chinese_enum_point(BeautifulSoup(item, "html.parser").get_text(" ", strip=True)) for item in fragments):
            continue
        ...
        for fragment in fragments:
            ...
            if _is_chinese_enum_point(text):
                new_p["data-enum-point"] = "true"
                dot = soup.new_tag("span")
                dot["style"] = (
                    "display:inline-block;margin-right:0.42em;color:#0f766e;"
                ...
                new_p.append(BeautifulSoup(fragment, "html.parser"))
        ...
        for new_node in reversed(new_nodes):
            p.insert_after(new_node)
        p.decompose()


def _decorate_callouts(soup: BeautifulSoup) -> None:
    for p in list(soup.find_all("p")):
        if p.find_parent(["li", "blockquote", "td", "th"]):
            continue
        text = p.get_text(" ", strip=True)
        key = _callout_style_key(text)
        if not key:
            continue

        style = CALLOUT_STYLES[key]
        section = soup.new_tag("section")
        section["style"] = (
            f"margin:1.05em 0;padding:0.86em 1em;border:1px solid {style['border']};"
            f"background:{style['bg']};border-radius:8px;"
        )
        label = soup.new_tag("section")
        label["style"] = (
            f"margin-bottom:0.35em;color:{style['text']};font-size:13px;"
            "line-height:1.25;font-weight:700;letter-spacing:0.08em;"
            "font-family:Georgia,'Times New Roman','PingFang SC',serif;"
        )
        label.string = style["label"]
        body = soup.new_tag("section")
        body["style"] = (
            f"color:{style['text']};font-size:17px;line-height:1.86;font-weight:700;"
            "font-family:'Songti SC','Noto Serif CJK SC','STSong','SimSun',serif;"
        )
        body.string = _strip_leading_marker(text)
        section.append(label)
        section.append(body)
        p.replace_with(section)


def _style_media_tables_and_rules(soup: BeautifulSoup, template: str) -> None:
    for img in soup.find_all("img"):
        ...
        parent = img.parent
        if getattr(parent, "name", None) == "p" and parent.get_text(" ", strip=True) == "":
            parent["style"] = "margin:1.35em 0 0.45em;text-align:center;line-height:0;"
            if alt and not parent.find_next_sibling(
                lambda tag: getattr(tag, "name", None) == "section"
                and tag.get("data-image-caption") == alt
            ):
                caption = soup.new_tag("section")
                caption["data-image-caption"] = alt
                caption["style"] = (
                    "margin:-0.1em 0 1.35em;color:#64748b;font-size:13px;"
    ...
    for table in soup.find_all("table"):
        table["style"] = (
            "width:100%;border-collapse:collapse;table-layout:auto;margin:1.25em 0;"
            "font-size:14px;line-height:1.65;"
        )
        for th in table.find_all("th"):
            th["style"] = (
                "padding:0.68em 0.55em;border:1px solid #dbe3ea;background:#0f766e;"
                "color:#ffffff;font-size:14px;font-weight:700;text-align:left;"
            )
        ...
        for row_index, tr in enumerate(rows):
            ...
            for td in tr.find_all("td"):
                td["style"] = (
                    f"padding:0.62em 0.55em;border:1px solid #e5e7eb;background:{bg};"
                    "color:#334155;font-size:14px;line-height:1.65;vertical-align:top;"
                )

    for hr in soup.find_all("hr"):
        divider = soup.new_tag("section")
        divider["style"] = "margin:1.75em auto;width:3.4em;border-top:3px solid #cbd5e1;"
...
def optimize_for_wechat_html(content_html: str, template: str = "standard") -> str:
    ...
    _split_chinese_enum_paragraphs(soup)
    _decorate_callouts(soup)
    _style_media_tables_and_rules(soup, template)

    # 处理 h1 标题（作为章节标题，保留但应用样式）
    # 第一个 h1 作为文章主标题删除，其余 h1 作为章节标题保留
    h1_list = soup.find_all("h1")
    if h1_list:
        # 删除第一个 h1（文章主标题）
        h1_list[0].decompose()
    
    # 为剩余的 h1 应用章节标题样式（飞书文档的# 格式）
    for h1 in soup.find_all("h1"):
        if template == "viral":
            h1["style"] = (
                "margin:1.6em 0 0.8em;padding:0.5em 0.8em;font-size:1.25em;line-height:1.5;"
                "color:#7a1f1f;font-weight:800;background:#fff2f2;border-left:4px solid #ef4444;border-radius:4px;"
            )
        else:
            h1["style"] = (
                "margin:1.5em 0 0.75em;padding:0.4em 0.7em;font-size:1.22em;line-height:1.48;"
                "color:#1f2937;font-weight:700;background:#f5f8ff;border-left:4px solid #3b6dd8;border-radius:2px;"
            )

    for h2_index, h2 in enumerate(soup.find_all("h2"), 1):
        if template == "viral":
            number = soup.new_tag("span")
            number["style"] = (
                "display:block;margin-bottom:0.32em;color:#0f766e;font-size:13px;line-height:1.2;"
                "font-weight:700;letter-spacing:0.1em;font-family:Georgia,'Times New Roman',serif;"
            )
            number.string = f"SECTION {h2_index:02d}"
            h2.insert(0, number)
            h2["style"] = (
                "margin:1.75em 0 0.82em;padding:0.72em 0.86em;font-size:21px;line-height:1.5;"
                "color:#0f172a;font-weight:800;background:#f0fdfa;border-left:5px solid #0f766e;border-radius:0 8px 8px 0;"
                "font-family:'Songti SC','Noto Serif CJK SC','STSong','SimSun',serif;"
            )
        else:
            h2["style"] = (
    ...
    # 处理 h3 小标题（飞书文档中的 ### 格式）
    # v2.1.2 修复：飞书文档的章节标题使用 ### 格式，需要应用样式才能正确显示
    for h3 in soup.find_all("h3"):
        if template == "viral":
            marker = soup.new_tag("span")
            marker["style"] = (
                "display:inline-block;width:0.45em;height:0.45em;margin-right:0.5em;"
                "border-radius:50%;background:#f97316;vertical-align:middle;"
            )
            h3.insert(0, marker)
            h3["style"] = (
                "margin:1.42em 0 0.68em;padding:0 0 0.42em;font-size:18px;line-height:1.48;"
                "color:#1e293b;font-weight:800;border-bottom:1px solid #e2e8f0;"
                "font-family:'PingFang SC','Hiragino Sans GB','Microsoft YaHei',sans-serif;letter-spacing:0.02em;"
            )
        else:
            h3["style"] = (
                "margin:1.3em 0 0.6em;padding:0.35em 0.6em;font-size:1.1em;line-height:1.4;"
                "color:#374151;font-weight:600;background:#f9fafb;border-left:3px solid #6b7280;border-radius:2px;"
            )
    ...
    for p in soup.find_all("p"):
        ...
        if p.get_text(" ", strip=True) in ("看点：", "看点:"):
            continue
        if p.get("data-enum-point") == "true":
            p["style"] = (
                "margin:0.62em 0;padding-left:0.1em;line-height:1.92;font-size:16px;color:#263241;"
                "text-align:justify;letter-spacing:0.02em;text-indent:0;"
                "font-family:-apple-system,BlinkMacSystemFont,'PingFang SC','Hiragino Sans GB','Microsoft YaHei',sans-serif;"
            )
        else:
            p["style"] = (
                "margin:0.95em 0;line-height:2.02;font-size:16px;color:#263241;"
                "text-align:justify;letter-spacing:0.02em;text-indent:0;"
                "font-family:-apple-system,BlinkMacSystemFont,'PingFang SC','Hiragino Sans GB','Microsoft YaHei',sans-serif;"
            )
    ...
    for li in soup.find_all("li"):
        li["style"] = (
            "margin:0.28em 0;font-size:16px;line-height:1.88;text-indent:0;"
            "font-family:-apple-system,BlinkMacSystemFont,'PingFang SC','Hiragino Sans GB','Microsoft YaHei',sans-serif;"
        )
    ...
    for blockquote in soup.find_all("blockquote"):
        if template == "viral":
            blockquote["style"] = (
                ...
                "background:#fff8e6;color:#5f4b20;line-height:1.92;border-radius:6px;"
                "font-family:'Songti SC','Noto Serif CJK SC','STSong','SimSun',serif;font-size:16px;"
            )
        else:
            blockquote["style"] = (
...
class WeChatClient:
    ...
    def get_token(self) -> str:
        ...
        raise WeChatPublishError(f"get_token failed: {last_error}")
